...
const Footer = ({ footerData }) => {
  return (
    <div className="bg-[#232233] w-full py-8 px-4 flex flex-col items-center gap-6 mt-[140px]">
      <CardContacto />
      <img src="logo.svg" alt="Logo de la empresa" className="w-24 h-auto md:w-32 lg:w-40" />
      ...
      <div className="flex flex-wrap justify-center gap-4 mt-6">
        ...
        {Object.entries(footerData.socialMedia).map(([key, { url }], index) => (
          <div className="flex items-center" key={key}>
            <a href={url} target="_blank" rel="noopener noreferrer" aria-label={`Ir a ${key}`} className="flex items-center">
              <Icon name={key} />
            </a>
      ...
      <div className="text-white mt-8 w-full max-w-md mx-auto">
        ...
        <form className="relative" onSubmit={(e) => e.preventDefault()}>
          ...
          <button 
            ...
            style={{ margin: '2px' }} // Ajuste de margen para separación
          >
            <Icon name="send" />
          </button>
        </form>
